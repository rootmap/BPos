...
class StaticDataController extends Facade {
    ...
    {
        //self::storeName()
        $count=\DB::table('stores')->where('id',Auth::user()->store_id)->count();
        if($count==0)
        {
            return "Retail Pos";
        }
    ...
    public static function PDFLayout($report_name,$table='')
    {
                ...
                <tr>
                    <td valign="top" style="border-bottom: 5px #000 solid; color: #008000; font-size: 20px; font-weight: bold; padding-left: 0px;">
                                '.$report_name.' : '.self::storeName().'<br />
                                <b>Report Genarated : '.date('Y-m-d H:i:s').'</b>
                                <hr style="height:5px;">
                    </td>
                <tr>
                </table>';
    ...
    )
    {
          ...
          try {
              ...
              $mail->setFrom('automail@simpleretailpos.com', self::storeName());

              //$mail->addAddress($to, 'Fahad Bhuyian');
              $mail->addAddress($to);               // Name is optional
              $mail->addReplyTo('support@simpleretailpos.com', 'Reply - '.self::storeName());
    ...
    public static function invoiceEmailTemplate($storeID=0,$UserID=0)
    {
        if(!empty($storeID) && !empty($UserID))
        {
            ...
            if($chk==0)
            {
                ...
                $tab->email_time=1;
                $tab->company_name=self::storeName();
